 ...
 * responses, mirroring how the Gemini path replays function calls.
 */
...
// ---------------------------------------------------------------------------
// Handlers (transport-agnostic: shared by the Express dev server and the
...
export async function handleChat(input: AiRequestInput): Promise<AiResult> {
  ...
  if (isRateLimited(input.ip || "unknown")) {
    return { status: 429, body: { error: "Too many requests. Please slow down.", code: "RATE_LIMITED" } };
  }

  const body = input.body ?? {};
  if (typeof body.messageText !== "string" || !body.messageText.trim()) {
    return { status: 400, body: { error: "messageText is required.", code: "BAD_REQUEST" } };
  }
  if (body.messageText.length > MAX_MESSAGE_CHARS) {
    return {
      status: 400,
      body: { error: `messageText must be under ${MAX_MESSAGE_CHARS} characters.`, code: "BAD_REQUEST" },
    };
  }
  ...
  let ctx: Ctx;
  try {
    ctx = buildContext(body);
  } catch {
    return { status: 400, body: { error: "Invalid context payload.", code: "BAD_REQUEST" } };
  }
...
export async function handleInsight(input: AiRequestInput): Promise<AiResult> {
  const apiKey = getApiKey();
  if (!apiKey) {
    return {
      status: 503,
      body: { error: "The AI coach is not configured on the server.", code: "AI_NOT_CONFIGURED" },
    };
  }
  ...
  const body = input.body ?? {};
  const statsContext = typeof body.statsContext === "string" ? body.statsContext.slice(0, 2000) : "";
  if (!statsContext.trim()) {
    return { status: 400, body: { error: "statsContext is required.", code: "BAD_REQUEST" } };
  }
  const language = body.language === "en" ? "en" : "id";
  ...
  try {
    ...
    return { status: 200, body: { content: stripThoughts(result.text || "") } };
  } catch (err: any) {
    const { status, code, error } = classifyError(err);
...
/** Best-effort client IP extraction for the in-memory rate limiter. */
export function clientIpFrom(headers: Record<string, any>, remoteAddress?: string): string {
